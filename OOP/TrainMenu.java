		...
		if (trainType == 1) {
			...
			while (itr.hasNext()) {
				...
				if ((object1.trainId==trainid)) {
					System.out.println("Enter no of seats");//tracking seats
...
System.out.println("AMOUNT---" + amount);
						...
						System.out.println("seats are less. Try another one");
		                menu();
		...
		if(trainType==2){
			...
			Iterator<GoodsTrain> itr = TrainDetails.goodsList
					.iterator();
			while (itr.hasNext()) {
				...
				if ((object1.trainId==trainid)) {
					...
					if (weight < object1.weight) {
						...
						int paymentType=sc.nextInt();
						PaymentMode paymentmode=new PaymentMode();//tracking payment
						if(paymentType==1)
						{
							paymentmode.creditCard(amount);
						}
